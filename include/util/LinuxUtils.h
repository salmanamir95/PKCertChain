...
UTIL_INLINE OpStatus_t load_keys_from_wallet(const char *priv_name,
                                             const char *pub_name,
                                             const char *password,
                                             size_t password_len,
                                             uint256 *out_priv,
 ...
 */
UTIL_INLINE OpStatus_t load_sign_keys(uint256 *out_priv,
                                      uint256 *out_pub,
                                      const char *password,
                                      size_t password_len)
{
    ...
    bool missing = false;
    OpStatus_t st = load_keys_from_wallet(PKCERTCHAIN_SIGN_PRIV_FILE, PKCERTCHAIN_SIGN_PUB_FILE,
                                          password, password_len, out_priv, out_pub, &missing);
    if (st != OP_SUCCESS) return st;
    if (!missing) return OP_SUCCESS;

    st = GenerateSignKeys(out_priv, out_pub);
    if (st != OP_SUCCESS) return st;
    return save_sign_keys(out_priv, out_pub, password, password_len);
}

/*
 * Load X25519 encryption keys. If missing, generate and save them.
 */
UTIL_INLINE OpStatus_t load_enc_keys(uint256 *out_priv,
                                     uint256 *out_pub,
                                     const char *password,
                                     size_t password_len)
{
    ...
    bool missing = false;
    OpStatus_t st = load_keys_from_wallet(PKCERTCHAIN_ENC_PRIV_FILE, PKCERTCHAIN_ENC_PUB_FILE,
                                          password, password_len, out_priv, out_pub, &missing);
    if (st != OP_SUCCESS) return st;
    if (!missing) return OP_SUCCESS;

    st = GenerateEncKeys(out_priv, out_pub);
    if (st != OP_SUCCESS) return st;
    return save_enc_keys(out_priv, out_pub, password, password_len);
}
