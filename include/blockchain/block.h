...
BLOCK_INLINE void block_init(block *blk)
{
    ...
    uint256_zero(&blk->SignedByVerifier);
    blk->height = 0;
    blk->timestamp = 0;
}
...
BLOCK_INLINE const uint256 *block_get_signed_by_verifier(const block *blk)
{
...
BLOCK_INLINE void block_set_signed_by_verifier(block *blk, const uint256 *signed_by_verifier)
{
    memcpy(&blk->SignedByVerifier, signed_by_verifier, UINT256_SIZE);
}
...
BLOCK_INLINE void block_copy(block *dst, const block *src)
{
    ...
    memcpy(&dst->SignedByVerifier, &src->SignedByVerifier, UINT256_SIZE);
...
BLOCK_INLINE OpStatus_t block_serialize(const block *blk, uint8_t *out, size_t out_size)
{
    ...
    if (uint256_serialize_be(&blk->SignedByVerifier, out + CERT_SIZE + UINT256_SIZE, UINT256_SIZE) != OP_SUCCESS) return OP_INVALID_INPUT;
    serialize_u64_be(blk->height, out + CERT_SIZE + 2 * UINT256_SIZE);
    serialize_u64_be(blk->timestamp, out + CERT_SIZE + 2 * UINT256_SIZE + UINT64_SIZE);
    return OP_SUCCESS;
}
...
BLOCK_INLINE OpStatus_t block_deserialize(const uint8_t *in, size_t in_size, block *blk)
{
    ...
    if (uint256_deserialize_be(in + CERT_SIZE + UINT256_SIZE, UINT256_SIZE, &blk->SignedByVerifier) != OP_SUCCESS) return OP_INVALID_INPUT;
    deserialize_u64_be(in + CERT_SIZE + 2 * UINT256_SIZE, &blk->height, sizeof(uint64_t));
    deserialize_u64_be(in + CERT_SIZE + 2 * UINT256_SIZE + UINT64_SIZE, &blk->timestamp, sizeof(uint64_t));
    return OP_SUCCESS;
}
